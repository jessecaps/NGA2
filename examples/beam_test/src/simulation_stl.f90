 ...
 contains
   ...
   subroutine simulation_init
      ...
      initialize_lss: block
         use mpi_f08,  only: MPI_ALLREDUCE,MPI_MAX,MPI_INTEGER
         real(WP) :: dx,mu,kk,max_stretch,Lx,Ly,Lz
         real(WP) :: xmin,xmax,ymin,ymax,zmin,zmax,ratio,dist,net_vol,center_dist,prev_center_dist
         real(WP), dimension(3) :: location
         integer :: np,nt,nx,ny,nz,ierr,global_index
         type triangle_type
            real(WP), dimension(3) :: norm
            real(WP), dimension(3) :: v1
         ...
         location=[0.5_WP,0.03_WP,0.03_WP]
         prev_center_dist=huge(1.0_WP)
         ...
         if (ls%cfg%amRoot) then
            ...
            read_bin: block
              ...
              do p=1,np
                 ...
                 ! Activate the particle
                 ls%p(p)%flag=0
                 center_dist=sqrt(dot_product((ls%p(p)%pos - location),(ls%p(p)%pos - location)))
                 if(center_dist < prev_center_dist) then
                   target_index = p
