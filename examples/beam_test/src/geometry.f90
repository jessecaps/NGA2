...
contains
   ...
   subroutine geometry_init
      ...
      create_grid: block
         ...
         real(WP) :: Lx,Ly,Lz,dist,dx
         real(WP), dimension(:), allocatable :: x,y,z
         
         Lx = 1.0_WP ! beam length
         dist = 0.01_WP ! Space between particles

         Lx = Lx + 3.0_WP * dist ! total length of the beam
         dx = 3.0_WP*dist ! grid spacing
         ...
         do i=1,nx+1
            x(i)=real(i-1,WP)*dx - 2.0_WP*dx
         end do
         do j=1,ny+1
            y(j)=real(j,WP)*dx-1.5_WP*dx
         end do
         do k=1,nz+1
            z(k)=real(k,WP)*dx-1.5_WP*dx
         end do
