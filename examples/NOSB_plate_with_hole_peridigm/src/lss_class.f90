...
contains
   ...
   subroutine get_bond_force(this)
      ...
      dim2d = 0
      if (this%cfg%nx.eq.1) dim2d = 1
      if (this%cfg%ny.eq.1) dim2d = 2
      if (this%cfg%nz.eq.1) dim2d = 3
      ...
      update_tensors: block
         ...
         do n1=1,this%np_
            ...
            if (is2D) then
               ! The row/column of K_mat associated with the degenerate
               ! direction is ~zero (particles don't vary in that
               ! direction), so the full 3x3 K is singular. Invert
               ! only the active in-plane 2x2 block, and set the
               ! out-of-plane row/column of K_inv to identity.
               select case (dim2d)
               case (1) ! x degenerate, active plane is (y,z)
                  detK = K_mat(2,2)*K_mat(3,3) - K_mat(2,3)*K_mat(3,2)
                  K_inv(1,1) = 1.0_WP
                  K_inv(2,2) =  K_mat(3,3)/detK
                  K_inv(2,3) = -K_mat(2,3)/detK
                  K_inv(3,2) = -K_mat(3,2)/detK
                  K_inv(3,3) =  K_mat(2,2)/detK
               case (2) ! y degenerate, active plane is (x,z)
                  detK = K_mat(1,1)*K_mat(3,3) - K_mat(1,3)*K_mat(3,1)
                  K_inv(2,2) = 1.0_WP
                  K_inv(1,1) =  K_mat(3,3)/detK
                  K_inv(1,3) = -K_mat(1,3)/detK
                  K_inv(3,1) = -K_mat(3,1)/detK
                  K_inv(3,3) =  K_mat(1,1)/detK
               case (3) ! z degenerate, active plane is (x,y)
                  detK = K_mat(1,1)*K_mat(2,2) - K_mat(1,2)*K_mat(2,1)
                  K_inv(3,3) = 1.0_WP
                  K_inv(1,1) =  K_mat(2,2)/detK
                  K_inv(1,2) = -K_mat(1,2)/detK
                  K_inv(2,1) = -K_mat(2,1)/detK
                  K_inv(2,2) =  K_mat(1,1)/detK
               end select
            else
            ...
            !if(detK.lt.1e-16_WP) K_inv = I_mat ! is this valid? What to do when this is small, 
                                                ! and is this the source of spurious movement? Very small when no displacement has occured

            
            p1%F = MATMUL(p1%F,K_inv)
            if (is2D) p1%F(dim2d,dim2d) = 1.0_WP
      ...
      update_bond_force: block
         ...
         integer :: i,j,k,n1,nn,n2,ierr
         type(part) :: p1,p2
         real(WP), dimension(3) :: rpos,t1,t2,tc1,tc2,xi,z
                     ...
                     do nn=1,npic(i,j,k)
                        ...
                        do nb=1,max_bond
                           if (p1%ibond(nb).eq.p2%i) then
                              ...
                              ! Force correction term (not formulated for 2D?)
                              ! z = rpos-MATMUL(p1%F,xi)
                              ! tc1 = w*(9.0_WP*kk/(Pi * this%delta**4))*(dot_product(xi,z)/(sqrt(dot_product(xi,xi)))**3)*xi       
                              ! z = rpos-MATMUL(p2%F,xi)
                              ! tc2 = w*(9.0_WP*kk/(Pi * this%delta**4))*(dot_product(xi,z)/(sqrt(dot_product(xi,xi)))**3)*xi       
                              ! Compute bond acceleration
                              !p1%Abond=p1%Abond+(t1+t2+tc1+tc2)*p2%vol/this%rho
                              p1%Abond=p1%Abond+(t1+t2)*p2%vol/this%rho
   ...
   subroutine advance(this,dt,stress_x,stress_y,stress_z)
      ...
      do n=1,this%np_
         ...
         ! Damping is put here, was in the thrombosis paper, unsure if should be
         if (this%p(n)%id.gt.-1) this%p(n)%vel=(1.0_WP-(this%beta*dt/2))*this%p(n)%vel+0.5_WP*dt*(this%gravity+this%p(n)%Abond+this%p(n)%Afluid)
      ...
      do n=1,this%np_
         ...
         ! Damping is put here, was in the thrombosis paper, unsure if should be
         this%p(n)%vel=this%p(n)%vel*(1.0_WP-(this%beta*dt/2))+0.5_WP*dt*(this%gravity+this%p(n)%Abond+this%p(n)%Afluid)
      end do
