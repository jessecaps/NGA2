 ...
 contains
   ...
   subroutine simulation_init
      ...
      initialize_lss: block
         ...
         ny = ceiling(Ly/dist)+1
         nz = ceiling(Lz/dist)+1
         nx = ceiling(Lx/dist)
         call param_read('Horizon Ratio',ratio)
         ls%delta = dist*ratio
         print*, ls%delta
         ...
         ! Only root process initializes solid particles
         if (ls%cfg%amRoot) then
            print *, (dist*dist*dist/ls%rho)
            ! Read the STL fiprint*, "here"
            read_bin: block
              ...
              do i=1,nx+3
                do j=1,ny
                  ...
                  !   if(ls%p(p)%id.eq.-1.and.ls%p(p)%pos(1).lt.0) ls%p(p)%id=-2
                     if(i.ge.(nx+3)-2) net_vol=net_vol+ls%p(p)%vol
                    ! Zero out force
                    ls%p(p)%Abond=0.0_WP
                    ! Zero out fluid unless end, using this for the load
                    ls%p(p)%Afluid=0.0_WP
                    if(i.ge.(nx+3)-2) ls%p(p)%Afluid=[(P_load/(dist**3 * ny * nz * 3))/(ls%rho),0.0_WP,0.0_WP]
                    ! Locate the particle on the mesh
                    ls%p(p)%ind=ls%cfg%get_ijk_global(ls%p(p)%pos,[ls%cfg%imin,ls%cfg%jmin,ls%cfg%kmin])
                    ! Assign a unique integer to particle
                    ls%p(p)%i=p
                    ! Activate the particle
                    ls%p(p)%flag=0
                    if(i.eq.(nx/2).and.j.eq.(ny/2).and.k.eq.(nz/2)) target_index = p
                  end do
                end do
              end do
     ...
     ! Create partmesh object for visualizing Lagrangian particles
      create_pmesh: block
         ...
         integer :: i,n,nbond
         pmesh=partmesh(nvar=6,nvec=3,name='solid')
