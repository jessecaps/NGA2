...
contains
   ...
   subroutine get_bond_force(this)
      ...
      update_weighted_vol_and_dilatation: block
         ...
         type(part) :: p1,p2
         integer :: nb,nbond
         real(WP), dimension(3) :: rpos, Gd_vec
         real(WP) :: dist, Gd_mag
         ! Loop over particles
         do n1=1,this%np_
            ! Create copy of our particle
      ...
      update_bond_force: block
         ...
         integer :: i,j,k,n1,nn,n2,ierr
         type(part) :: p1,p2
         real(WP), dimension(3) :: rpos,t12,t21,Gd_vec,f
         real(WP) :: dist,beta,alpha,ed,t,Gd_mag
         ...
         do n1=1,this%np_
                     ...
                     do nn=1,npic(i,j,k)
                        ...
                        do nb=1,max_bond
                           if (p1%ibond(nb).eq.p2%i) then
                              ...
                              p1%sigma(2)=p1%sigma(2) + 0.5_WP*f(2)*rpos(2)*p2%vol
                              p1%sigma(3)=p1%sigma(3) + 0.5_WP*f(3)*rpos(3)*p2%vol
                              p1%sigma(4)=p1%sigma(4) + 0.25_WP*(f(1)*rpos(2) + f(2)*rpos(1))*p2%vol
                              p1%sigma(5)=p1%sigma(5) + 0.25_WP*(f(1)*rpos(3) + f(3)*rpos(1))*p2%vol
                              p1%sigma(6)=p1%sigma(6) + 0.25_WP*(f(2)*rpos(3) + f(3)*rpos(2))*p2%vol
            ...
            p1%sigma=p1%sigma/p1%vol
            !p1%vonMises=sqrt(((p1%sigma(1)-p1%sigma(2))**2 + (p1%sigma(1)-p1%sigma(3))**2 + (p1%sigma(3)-p1%sigma(2))**2 + 6.0_WP*(p1%sigma(4)**2 + p1%sigma(5)**2 + p1%sigma(6)**2))/2.0_WP)
            p1%vonMises = sqrt(p1%vonMises)
            ! Copy back the particle
            this%p(n1)=p1
         end do
         ! Get global minimum
   ...
   subroutine advance(this,dt)!,stress_x,stress_y,stress_z)
      ...
      do n=1,this%np_
         ...
         if (this%p(n)%id.gt.-1) this%p(n)%vel=(1-this%beta)*this%p(n)%vel+0.5_WP*dt*(this%gravity+this%p(n)%Abond+this%p(n)%Afluid)
      ...
      do n=1,this%np_
         ...
         this%p(n)%vel=this%p(n)%vel*(1-this%beta)+0.5_WP*dt*(this%gravity+this%p(n)%Abond+this%p(n)%Afluid)
      end do
